...
-- | Message on the crumbs
data OnCrumb = OnCrumb (Crumb Split -> Crumb Split)
             | Swap
...
setAxis :: Axis -> Split -> Split
setAxis a (Split _ r) = Split a r

setAxisCrumbMessage :: Axis -> OnCrumb
setAxisCrumbMessage a = OnCrumb (modifyParentVal (setAxis a))
...
crumbCurrent :: (Crumb a -> Crumb a) -> Zipper a -> Maybe (Zipper a)
crumbCurrent _ l@(_, []) = Just l
crumbCurrent k (n, c:cs) = Just (n, k c:cs)
...
onNth :: (Zipper Split -> Maybe (Zipper Split)) -> BinarySpacePartition a -> BinarySpacePartition a
onNth _ (BinarySpacePartition _ _ _ Nothing) = emptyBSP
onNth _ (BinarySpacePartition _ _ _ (Just (Leaf _))) = emptyBSP
onNth f b = doToNth f b

removeNth :: BinarySpacePartition a -> BinarySpacePartition a
removeNth b = onNth removeCurrent b
  ...
  handleMessage b_orig m
   ...
   | otherwise = do
    ...
    where handleMesg r = msum [ fmap resize        (fromMessage m)
          ...
          onCrumb cm = resetFoc (onNth (crumbCurrent (crumbFunc cm)) b)

          crumbFunc Rotate      = modifyParentVal oppositeSplit
          crumbFunc Swap        = swapCrumb
          crumbFunc (OnCrumb k) = k
